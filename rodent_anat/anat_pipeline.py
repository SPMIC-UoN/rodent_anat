...
def run(options):
    ...
    if options.strongbias:
        LOG.info(" - Using strongbias option (NOT FUNCTIONAL YET)")
    if options.nocrop:
        LOG.info(" - Automated cropping is disabled (NOT FUNCTIONAL YET)")
    if options.noreorient:
        LOG.info(" - Reorientation of images is disabled")
    if options.nobias:
        LOG.info(" - Bias-field correction is disabled")
    if options.noreg:
        LOG.info(" - Standard space registration is disabled (NOT FUNCTIONAL YET)")
    if options.nononlinreg:
        LOG.info(" - Nonlinear standard space registration is disabled (NOT FUNCTIONAL YET)")
    if options.noseg:
        LOG.info(" - Tissue segmentation is disabled (NOT FUNCTIONAL YET)")
    if options.nosearch:
        LOG.info(" - Linear registration will use -nosearch (NOT FUNCTIONAL YET)")
    LOG.info(f" - Bias field smoothing parameter: {options.biassmooth}mm")
    LOG.info(f" - Non-linear registration using: {options.nonlinreg}")
    LOG.info(f" - Template data from: {options.template}")
    LOG.info(f" - ANTS location: {options.antsdir}")
    LOG.info(f" - MMORF location: {options.mmorfdir}")
    LOG.info(f" - C3D location: {options.c3ddir}")
    ...
    with working_dir(outdir):
        ...
        if options.nonlinreg == "ants":
            ...
            os.environ["ANTSPATH"] = options.antsdir
            ...
            os.system(f"{options.antsdir}/antsRegistrationSyN.sh -d 3 -f {templ_brain} -m T2_brain_templ_linear.nii.gz -j 1 -o ants/T2_to_templ_ -x {templ_mask_dil} |tee ants/ants.log")

            LOG.info(" - Converting ANTs transforms so that they are compatible with FSL (ITK transform (RAS) matrix to FSL xfm)")
            os.system(f"{options.c3ddir}/c3d_affine_tool -ref {templ_brain} -src T2_brain_templ_linear.nii.gz -itk ants/T2_to_templ_0GenericAffine.mat -ras2fsl -o ants/T2_to_templ_linear.mat")

            LOG.info(" - Performing multicomponent split (-mcs)")
            os.system(f"{options.c3ddir}/c3d -mcs ants/T2_to_templ_1Warp.nii.gz -oo ants/wx.nii.gz ants/wy.nii.gz ants/wz.nii.gz")

            LOG.info(" - Flipping warp")
            fsl.fslmaths("ants/wy").mul(-1).run("ants/i_wy")

            LOG.info(" - Concatenating components to give whole warp")
            # FIXME do this in python?
            ...
            fsl.invwarp("ants/T2_to_templ_warp_combined", "T2_brain_templ_linear", out="ants/templ_to_T2_warp_combined")
            ...
            cmd = f"singularity run --nv {options.mmorfdir}/mmorf.sif --warp_out mmorf/T2_to_templ_warp --jac_det_out mmorf/T2_to_templ_jac --bias_out mmorf/T2_to_templ_bias --config mmorf/mmorf_config.ini 2>&1 |tee mmorf/mmorf.log"
            LOG.debug(cmd)
            os.system(cmd)
