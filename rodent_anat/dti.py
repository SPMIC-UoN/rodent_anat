...
def standardize_dti(fpath):
    ...
    new_bval = np.round(bval, -3)

    # Handle repeats in BVALs and BVECs
    n_bvecs = new_bvec.shape[1]
    ...
    new_bvec = np.tile(new_bvec, num_rpts)
    new_bval = np.tile(new_bval, num_rpts)
...
def fix_dti_orientation(fpath, anat_fpath, use_fname=True):
    ...
    """
    with tempfile.NamedTemporaryDirectory() as d:
        LOG.info(f"Testing {fpath} for fwd/rev orientation")
        fwd = os.path.join(d.name, "fwd.nii.gz") # FIXME extension
        rev = os.path.join(d.name, "rev.nii.gz")
